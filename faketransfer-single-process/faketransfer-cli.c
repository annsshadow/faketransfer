...
int main(int argc, const char *argv[])
{
    ...
    if( argc != 6 )
    {
        printf("Usage:%s server-IP upload/download filename source_path destination_path\n", argv[0]);
        return 0;
    }

    if( strlen(argv[2]) + 1 > COMMAND_SIZE )
    {
        printf("Usage:can only use command:upload ordownload\n");
        return 0;
    }
    ...
    //check the command
    if( strncmp(command_buf, "upload", sizeof("upload")) != 0 && strncmp(command_buf, "download", sizeof("download")) != 0 )
    {
    ...
    //download file
    else if( strncmp(command_buf, "download", sizeof(command_buf)) == 0 )
    {
        int ret = fake_client_downloadfile(m_socketfd, src_file_path, dst_file_path);
        if( ret == 1 )
            printf("Download Success--->%s\n", filename_buf);
        else if( ret == 0 )
        {
            printf("Download Interupt--->%s\n", filename_buf);
        }
        else
        {
